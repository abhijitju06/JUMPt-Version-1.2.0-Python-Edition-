...
# -*- coding: utf-8 -*-
"""
JUMPt Python — Setting-2 (HOURS mode)

Run:
  python jumpt_hours.py --params JUMPt_main.params
"""
...
def parse_params(path: str) -> dict:
    ...
    Values coerced to int/float where possible, else left as strings.
    """
    def coerce_scalar(s: str):
        ...
        if re.fullmatch(r"[-+]?(?:\d*\.\d+|\d+\.\d*|\d+)(?:[eE][-+]?\d+)?", s or ""):
            try: return float(s)
            except: pass
        return s
    ...
    P.setdefault("purity_of_SILAC_food", 99.0)   # %
    P.setdefault("alphaUB", 0.1)
    P.setdefault("disable_physics_filter", 0)
    P.setdefault("physics_filter_mode", "clip")  # clip|drop|none
    P.setdefault("min_observations_per_protein", 3)
    P.setdefault("free_lys_weight_mode", "auto") # auto|matlab|numeric
    P.setdefault("initial_condition_mode", "ones") # ones|data
    P.setdefault("apparent_T50_calculation", 1)
    P.setdefault("random_seed", 12345)
    P.setdefault("n_starts", 6)                  # speed default
    P.setdefault("max_nfev_global", 400)         # speed default
    P.setdefault("max_nfev_refine", 200)         # speed default
    P.setdefault("use_refine", 1)                # 1 = LM refine; 0 = skip
    P.setdefault("sort_by_apparent_t50", 1)

    # HOURS display defaults (NOT clamps)
    P.setdefault("hl_tmax_hours", 2400.0)
    P.setdefault("display_min_hours", 0.5)
    P.setdefault("display_max_hours", 2400.0)

    # Numerical safety: internal min half-life (keeps gamma finite) & expm step size
    P.setdefault("internal_min_hours",
                 max(1e-3, float(P.get("display_min_hours", 0.5))/20.0))  # ~0.025 h
    P.setdefault("expm_safe_magnitude", 400.0)  # fewer substeps than 50 → much faster
...
def _find_time_row(df: pd.DataFrame, tcols: list) -> int:
...
def _find_free_lys_row(df: pd.DataFrame, tcols: list, after_row: int) -> int:
    ...
    for r in range(after_row+1, min(after_row+10, len(df))):
        ...
        sl = s.lower()
        if ("free" in sl and "lys" in sl) or ("lysine" in sl):
            return r
...
def _read_input_like_matlab(xlsx_path: str):
    ...
    if not tcols:
        raise ValueError("No 'time_point*' columns found.")
...
class BinBuild:
    data: DataStruct
    free_lys_weight_fn: Callable[[int], float]
...
def build_binning(P: dict) -> BinBuild:
    ...
    if phys_enabled:
        SILAC, keep_mask = apply_physics_filter(LysRatio, SILAC.copy(),
                                                str(P.get("physics_filter_mode","clip")), min_obs)
    ...
    if int(P.get("sort_by_apparent_t50", 1)) == 1:
    ...
    Ansatz_ = rng.rand(int(P.get("n_starts", 6)), N + 2)
    ...
    def free_lys_weight_for_bin(m: int) -> float:
        if wmode == "auto":  return float(m)
        if wmode in ("matlab","auto_div5"): return max(1.0, m/5.0)
        try: return float(wmode)
        except: return float(m)

    data = DataStruct(
        t=t.astype(float), LysRatio=LysRatio.astype(float),
        ProtInfo=ProtInfo, SILAC_data=SILAC.astype(float),
        timepoints=tp_cols, protBins=protBins, Ansatz_=Ansatz_,
        physics_filter_mode=str(P.get("physics_filter_mode","clip")),
        free_lys_weight_mode=wmode, y0_mode=y0_mode
    )
    return BinBuild(data=data, free_lys_weight_fn=free_lys_weight_for_bin)

# =====================================================================
# Model, numerics, fitting
# =====================================================================
def simulate_lti_exact(t: np.ndarray, A: np.ndarray, b: np.ndarray, y0: np.ndarray,
                       safe_mag: float = 400.0) -> np.ndarray:
    """
    Exact affine solution via block-matrix exponential, with adaptive sub-stepping
    so that ||M||*(dt/s) <= safe_mag. Larger safe_mag → fewer steps (faster).
    """
    n = A.shape[0]
    ...
    for k in range(1, t.size):
        ...
        s = int(max(1, math.ceil((normM * dt) / max(safe_mag, 1e-6))))
...
def build_A_b_from_gamma(p_gamma: np.ndarray, theta_food_impurity: float):
    """p_gamma = [gamma_A, gammas(1..m), alpha]."""
...
def _initial_y0(mode: str, Y_obs_first: np.ndarray) -> np.ndarray:
    if mode == "data":
        ...
        return y0
    return np.ones_like(Y_obs_first, dtype=float)

def weighted_residuals_gamma(p: np.ndarray, t: np.ndarray, Y_obs: np.ndarray,
                             theta_food_imp: float, ub: np.ndarray, wF: float,
                             y0_mode: str, safe_mag: float) -> np.ndarray:
    p = np.clip(p, 0, ub)
    A, b = build_A_b_from_gamma(p, theta_food_imp)
    y0 = _initial_y0(y0_mode, Y_obs[0, :])
    Y_sim = simulate_lti_exact(t, A, b, y0, safe_mag=safe_mag)
    R = (Y_sim - Y_obs)
    R[:, 0] *= wF
    return R[np.isfinite(R)]

def _format_halflife_hours(hl_h: float, min_h: float, max_h: float) -> str:
    if not np.isfinite(hl_h):
        return "NA"
    if hl_h < min_h:
        return f"<{min_h:g} h"
    if hl_h > max_h:
...
def fit_corrected(binbuild: BinBuild, P: dict) -> pd.DataFrame:
    ...
    # q-space bounds (log-gamma) for HOURS
    hl_tmax_h = float(P.get("hl_tmax_hours", 2400.0))
    internal_min_h = float(P.get("internal_min_hours", 0.025))
    q_lo = math.log(max(LN2 / hl_tmax_h, 1e-300)) if (hl_tmax_h and np.isfinite(hl_tmax_h)) else -30.0
    q_hi = math.log(LN2 / internal_min_h)

    alphaUB  = float(P["alphaUB"])
    SAFE_MAG = float(P.get("expm_safe_magnitude", 400.0))
    use_refine = int(P.get("use_refine", 1))

    rows = []
    for bi in range(len(data.protBins)-1):
        lo = data.protBins[bi]; hi = data.protBins[bi+1]; m = hi - lo
        if m <= 0: continue
        print(f"  [hours] Optimizing proteins {lo+1} to {hi} (of {data.protBins[-1]})")
        Y_obs = np.c_[data.LysRatio, data.SILAC_data[:, lo:hi]]

        ub_g = np.exp(q_hi)
        ub   = np.r_[np.full(1+m, ub_g), np.array([alphaUB], float)]
        wF   = wF_of_bin(m)

        rng = np.random.RandomState(int(P.get("random_seed", 12345)))
        S = int(P.get("n_starts", 6))
        qA0 = rng.uniform(q_lo, q_hi, size=(S, 1))
        qP0 = rng.uniform(q_lo, q_hi, size=(S, m))
        a0  = rng.uniform(0.0, alphaUB, size=(S, 1))
        starts = np.hstack([np.exp(qA0), np.exp(qP0), a0])

        best = None; best_free_sse = np.inf
        for s in starts:
            res = least_squares(
                weighted_residuals_gamma, s,
                args=(data.t, Y_obs, impurity, ub, wF, data.y0_mode, SAFE_MAG),
                method="trf", bounds=(np.zeros_like(ub), ub),
                xtol=1e-10, ftol=1e-10, gtol=1e-10,
                max_nfev=int(P.get("max_nfev_global", 400)), verbose=0
            )
            pg = np.clip(res.x, 0, ub)
            A, b = build_A_b_from_gamma(pg, impurity)
            y0 = _initial_y0(data.y0_mode, Y_obs[0, :])
            Ysim = simulate_lti_exact(data.t, A, b, y0, safe_mag=SAFE_MAG)
            free_sse = float(np.nansum((Ysim[:, 0] - data.LysRatio)**2))
            if free_sse < best_free_sse:
                best_free_sse = free_sse; best = pg

        # --- optional refine in q-space ---
        alpha = float(best[-1])
        q0 = np.log(np.maximum(best[:-1], 1e-300))
        q0 = np.clip(q0, q_lo, q_hi)

        def resid_q(q):
            q = np.clip(q, q_lo, q_hi)
            pg = np.exp(np.clip(q, -50.0, 50.0))
            p_full = np.r_[pg, alpha]
            return weighted_residuals_gamma(p_full, data.t, Y_obs, impurity, ub, wF, data.y0_mode, SAFE_MAG)

        if use_refine:
            try:
                res2 = least_squares(resid_q, q0, method="lm",
                                     xtol=1e-12, ftol=1e-12, gtol=1e-12,
                                     max_nfev=int(P.get("max_nfev_refine", 200)))
            except Exception:
                res2 = least_squares(resid_q, q0, method="trf",
                                     bounds=(np.full_like(q0, q_lo), np.full_like(q0, q_hi)),
                                     xtol=1e-12, ftol=1e-12, gtol=1e-12,
                                     max_nfev=int(P.get("max_nfev_refine", 200)))
            qhat = np.clip(res2.x, q_lo, q_hi)
            J = res2.jac
            dof = max(res2.fun.size - res2.x.size, 1)
        ...
        pg_hat = np.exp(np.clip(qhat, -50.0, 50.0))
        p_full = np.r_[pg_hat, alpha]
        ...
        HL_A = float(LN2 / max(gA, 1e-300))
        HLs  = LN2 / np.maximum(gP, 1e-300)

        disp_min = float(P.get("display_min_hours", 0.5))
        disp_max = float(P.get("display_max_hours", 2400.0))

        try:
            ci_halfwidth = np.abs(np.r_[HL_A, HLs]) * (ci_q_half if np.ndim(ci_q_half)==1 else np.full_like(qhat, np.nan))
        except Exception:
            ci_halfwidth = np.full(1+m, np.nan, float)

        for k in range(m):
            hl = float(HLs[k])
            disp = _format_halflife_hours(hl, disp_min, disp_max)
            extreme = 1 if (hl < disp_min or hl > disp_max) else 0
            rows.append([
...
def export_apparent(binbuild: BinBuild, input_file: str):
    ...
    base = {}
    if len(cols) >= 1: base["Protein"] = data.ProtInfo.iloc[order, 0].astype(str).to_numpy()
    if len(cols) >= 2: base["Gene"]    = data.ProtInfo.iloc[order, 1].astype(str).to_numpy()
    out_app = pd.DataFrame(base)
    out_app["Apparent T50 (hours)"] = t50[order]
    out_path = f"results_Apparent_T50_{os.path.basename(input_file)}"
    out_app.to_excel(out_path, index=False)
    print(f"  Apparent T50 (hours) → {out_path}")
...
def main(params_path: str):
    ...
    with pd.ExcelWriter(out_corr) as w:
        pd.DataFrame([{
            ...
            "use_refine": int(P["use_refine"]),
            "alphaUB": float(P["alphaUB"]),
            "hl_tmax_hours": float(P.get("hl_tmax_hours", 2400.0)),
            "display_min_hours": float(P.get("display_min_hours", 0.5)),
            "display_max_hours": float(P.get("display_max_hours", 2400.0)),
            "internal_min_hours": float(P.get("internal_min_hours", 0.025)),
            "expm_safe_magnitude": float(P.get("expm_safe_magnitude", 400.0)),
